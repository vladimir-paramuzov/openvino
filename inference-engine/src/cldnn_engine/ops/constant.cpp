...
void Program::CreateConstantOp(cldnn::topology& topology, const std::shared_ptr<ngraph::Node>& node) {
    ...
    if (bufIter != blobMemCache.end()) {
        ...
        topology.add(cldnn::data(initialconstPrimID, mem));
        blobMemCache[data] = initialconstPrimID;
        constPrimID = initialconstPrimID;
    }

    AddPrimitiveToProfiler(op, constPrimID);
}
