...
void Program::CreateProposalOp(cldnn::topology& topology, const std::shared_ptr<ngraph::Node>& node) {
    ...
    if (op->get_output_size() == 2) {
        ...
        primitivesToIRLayersMap[proposal_mutable_id_w] = { op->get_friendly_name() };
        ...
        primitivesToIRLayersMap[proposal_mutable_id_r] = { op->get_friendly_name() };
        primitiveIDs[proposal_mutable_id_r] = proposal_mutable_id_r;
        topology.add(argmax_mutable_prim_r);

        AddPrimitiveToProfiler(proposalLayerName, op);
        return;
    }
