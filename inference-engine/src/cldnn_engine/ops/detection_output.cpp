...
void Program::CreateDetectionOutputOp(cldnn::topology& topology, const std::shared_ptr<ngraph::Node>& node) {
    auto op = std::dynamic_pointer_cast<ngraph::op::v0::DetectionOutput>(node);
    if (!op)
