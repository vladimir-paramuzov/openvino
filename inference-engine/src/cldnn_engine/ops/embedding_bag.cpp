...
void Program::CreateEmbeddingBagOffsetsSumOp(cldnn::topology& topology, const std::shared_ptr<ngraph::Node>& node) {
    auto op = std::dynamic_pointer_cast<ngraph::op::v3::EmbeddingBagOffsetsSum>(node);
    if (!op)
        THROW_IE_EXCEPTION << INVALID_OP_MESSAGE;

    ValidateInputs(op, {3, 4, 5});
...
void Program::CreateEmbeddingBagPackedSumOp(cldnn::topology& topology, const std::shared_ptr<ngraph::Node>& node) {
...
void Program::CreateEmbeddingSegmentsSumOp(cldnn::topology& topology, const std::shared_ptr<ngraph::Node>& node) {
    auto op = std::dynamic_pointer_cast<ngraph::op::v3::EmbeddingSegmentsSum>(node);
    if (!op)
        THROW_IE_EXCEPTION << INVALID_OP_MESSAGE;

    ValidateInputs(op, {4, 5, 6});
    auto inputPrimitives = GetInputPrimitiveIDs(op);
    std::string layerName = layer_type_name_ID(op);
