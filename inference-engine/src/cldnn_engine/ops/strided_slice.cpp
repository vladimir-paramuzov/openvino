...
void Program::CreateStridedSliceOp(cldnn::topology& topology, const std::shared_ptr<ngraph::Node>& node) {
    ...
    do {
        ...
        if (!shrink_axis_mask.empty()) {
            ...
            topology.add(reshapePrim);
            AddInnerPrimitiveToProfiler(reshapeOutName, layerName, op);
        }
        return;
    } while (false);
