...
void Program::CreateCustomOp(cldnn::topology& topology, const std::shared_ptr<ngraph::Node>& op, CLDNNCustomLayerPtr customLayer) {
        ...
        case CLDNNCustomLayer::ParamType::Input: {
            ...
            if (param.portIndex < inputPrimitives.size() && reorderedInputs[param.portIndex].empty()) {
                ...
                if (param.format != cldnn::format::any) {
                    auto reorderPrimName = inputPrimitives[param.portIndex] + "_" + op->get_friendly_name() + m_preCustomLayerTag;
    ...
    AddPrimitiveToProfiler(genericLayerName, op);
