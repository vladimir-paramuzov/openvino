...
void Program::CreateMatMulOp(cldnn::topology& topology, const std::shared_ptr<ngraph::Node>& node) {
    auto op = std::dynamic_pointer_cast<ngraph::op::v0::MatMul>(node);
    if (!op)
    ...
    if (is_fc) {
        ...
        if (!op->get_transpose_b()) {
            ...
            AddInnerPrimitiveToProfiler(permuteName, layerName, op);
        ...
        bool reshape_3d_case = shape_a_aligned.size() == 3;
        ...
        auto reshape_to_2d = [&](const ngraph::Shape& shape, std::string inputName, size_t features, std::string suffix) -> std::string {
            ...
            AddInnerPrimitiveToProfiler(reshapeInName, layerName, op);
            return reshapeInName;
        };

        // TODO: Remove it once 3D FC is supported directly
        if (reshape_3d_case) {
        ...
        if (reshape_3d_case) {
            ...
            AddInnerPrimitiveToProfiler(outReshapeName, layerName, op);

            lastLayerName = outReshapeName;
        }

        AddPrimitiveToProfiler(op, lastLayerName);
    } else {
