...
std::string FusedOpsCodeGenerator::GetJitLoad(const FusedOpsConfiguration& conf, size_t input_id, const DataTensor prim_output,
    ...
    bool invalid_broadcast_case = prim_output.LogicalSize() == input_tensor.LogicalSize() &&
                                  prim_output.Feature().v != input_tensor.LogicalSize() &&
    ...
    if (desc.GetType() == KernelType::ELTWISE && invalid_broadcast_case &&
