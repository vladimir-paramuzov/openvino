...
std::vector<layout> resample_inst::calc_output_layouts(resample_node const& node, const kernel_impl_params& impl_param) {
    ...
    if (memory_deps.empty() && desc->output_pattern.empty()) {
        return { layout{desc->output_partial_shape, input_layout.data_type, format::adjust_to_rank(input_layout.format, desc->output_partial_shape.size())} };
    }

    ov::op::v4::Interpolate op;
    op.set_attrs(desc->get_attrs());

    ShapeType pattern_shape = impl_param.input_layouts.size() == 2 ? impl_param.input_layouts[1].get_partial_shape()
